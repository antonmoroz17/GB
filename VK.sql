...
DROP TABLE IF EXISTS users; -- создаст объект в любом случае если даже уже существует
CREATE TABLE users (
	id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY, 
    firstname VARCHAR(50),
    lastname VARCHAR(50) COMMENT 'Фамилия', -- COMMENT на случай, если имя неочевидное
    email VARCHAR(120) UNIQUE,
 	password_hash VARCHAR(100), -- _hash скрытый вид хранения данных	
...
ALTER TABLE `profiles` ADD CONSTRAINT fk_user_id
    ...
    ON UPDATE CASCADE -- (значение по умолчанию индексирование ИД автоматически)
...
creat table status (
    ...
    FOREIGN KEY (status_user_id) REFERENCES users(id)
);
